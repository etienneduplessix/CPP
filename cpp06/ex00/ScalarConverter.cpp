...
int ScalarConverter::checkDigit(double temp, const std::string& toConvert) {
    if (temp / 10 == 0 && toConvert.find('.') == std::string::npos)
        return 1;
    return 0;
}

int ScalarConverter::parseString(const std::string& toConvert) {
    if (toConvert.empty())
        return -1;
    else if (toConvert.length() == 1 && (single(toConvert) != -1))
        return type;
    else if (pseudoLiterals(toConvert) == 4)
        return type;
    ...
    if (checkDigit(temp, toConvert) == 1 || errno == ERANGE || errno == EDOM || errno == EILSEQ)
        return -1;
    if (checkInt(temp, toConvert) == 1)
        return type;
    else if (checkFloat(temp, toConvert) == 2)
        return type;
    else if (checkDouble(temp, toConvert) == 3)
        return type;
    return 0;
}
...
void ScalarConverter::fromFloat() {
    if (theFloat <= 126 && theFloat >= 32) {
        ...
        std::cout << "Char: impossible" << std::endl;
    if (theFloat > 2147483647.0f || theFloat < -2147483648.0f)
...
int ScalarConverter::checkInt(double temp, const std::string& toConvert) {
...
int ScalarConverter::checkFloat(double temp, const std::string& toConvert) {
...
int ScalarConverter::checkDouble(double temp, const std::string& toConvert) {
...
int ScalarConverter::single(const std::string& toConvert) {
    if (isdigit(toConvert.c_str()[0])) {
        type = 1;
        theInt = atoi(toConvert.c_str());
        return 1;
    }
    else if (isprint(toConvert.c_str()[0])) {
        type = 0;
        theChar = toConvert[0];
        return 0;
    }
...
int ScalarConverter::pseudoLiterals(const std::string& toConvert) {
    const std::string pseudoFloat[3] = {"-inff", "+inff", "nanf"};
    const std::string pseudoDouble[3] = {"-inf", "+inf", "nan"};
    int i;
    ...
    if (i < 3) {
        theFloat = static_cast<float>(strtod(pseudoFloat[i].c_str(), NULL));
        theDouble = static_cast<double>(strtod(pseudoDouble[i].c_str(), NULL));
        type = 4;
        return 4;
    }
    return 0;
